...
{
	// get the datatype from reg_descriptor and do stuff.
	return node_data_type ; 
}
...
{
	int t1 = actual_param_list.size() ;
	int t2 = formal_param_list.get_table().size() ;
	// cout << t1 << " " << t2 << endl ; 
	if(t1 != t2)
	{
		printf("cs316: Error function parameters size mismatch\n") ;
		exit(-1);
	}
	else
	{
		list<Ast*>:: iterator i1 = actual_param_list.begin() ;
		list<Symbol_Table_Entry *>:: iterator i2 = formal_param_list.get_table().begin() ;
		for(; i1 != actual_param_list.end(); i1++, i2++)
		{
			if((*i1) -> get_data_type() != (*i2) -> get_data_type())
			{
				printf("cs316: Error function parameter data types mismatch\n") ;
				exit(-1);
			}
		}
	}
