...
Code_For_Ast & Return_Ast::compile()
{
	...
	list<Icode_Stmt *> Rh = Ret.get_icode_list() ; 
	for(list<Icode_Stmt *>:: iterator i = Rh.begin() ; i != Rh.end() ; i++)
		{
			// printf("%d\n", 1 );
			Comb.push_back(*i) ; 
	...
	Register_Descriptor *Reg1 ; 
	Register_Descriptor *Reg2 = Ret.get_reg() ;
	if(node_data_type == int_data_type)
	{
		Reg1 = machine_desc_object.spim_register_table[v1] ;
		Register_Addr_Opd* result1 = new Register_Addr_Opd(Reg1) ; 		
		// Mem_Addr_Opd* Mem = new Mem_Addr_Opd(var -> get_symbol_entry()) ; 
		Register_Addr_Opd* result2 = new Register_Addr_Opd(Reg2) ;	
		Move_IC_Stmt* N = new Move_IC_Stmt(mov,result2,result1) ;
		Comb.push_back(N) ;
	}
	else if(node_data_type == double_data_type)
	{
		...
		// Mem_Addr_Opd* Mem = new Mem_Addr_Opd(var -> get_symbol_entry()) ; 
		Register_Addr_Opd* result2 = new Register_Addr_Opd(Reg2) ;	
		Move_IC_Stmt* N = new Move_IC_Stmt(move_d,result2,result1) ;
		Comb.push_back(N) ;	
	}
	if(node_data_type != void_data_type)
		Reg2 -> reset_use_for_expr_result() ; 
	string ret_f = "epilogue_" + proc_name ;
 	Label_IC_Stmt* Ret_i = new Label_IC_Stmt(j , ret_f);
	Comb.push_back(Ret_i) ;
	Code_For_Ast* C = new Code_For_Ast(Comb, Reg1); // set the return value to be this. 
...
Code_For_Ast & Conditional_Expression_Ast::compile()
{
	...
	// add check
	Register_Addr_Opd* Zero = new Register_Addr_Opd(machine_desc_object.spim_register_table[zero]) ; 


	Control_Flow_IC_Stmt* Jump = new Control_Flow_IC_Stmt(required_op , Reg , Zero, right_side ) ; // what label name
	Comb.push_back(Jump) ;  
...
Code_For_Ast & Selection_Statement_Ast::compile()
{
	...
	Register_Addr_Opd* Zero = new Register_Addr_Opd(machine_desc_object.spim_register_table[zero]) ; 
	Control_Flow_IC_Stmt* Jump = new Control_Flow_IC_Stmt(required_op , Reg , Zero,  else_label ) ; // what label name
...
Code_For_Ast & Iteration_Statement_Ast::compile()
{
	...
	Control_Flow_IC_Stmt* Jump = new Control_Flow_IC_Stmt(required_op , Reg , Zero, body_label ) ;
	Comb.push_back(Jump) ;
	// release the registers
...
Code_For_Ast & Print_Ast::compile()
{
	...
	Control_Flow_IC_Stmt* Print = new Control_Flow_IC_Stmt(Tgt_Op::print, NULL, NULL,  "") ;
...
Code_For_Ast & Call_Ast::compile()
{
    ...
    std::list<Symbol_Table_Entry *> formal_list = P -> get_formal_param_list().get_table() ; // reverse.
    formal_list.reverse() ; // reverse; 
    std::list<Symbol_Table_Entry *>::iterator it1 = formal_list.begin();

    // x = P -> get_formal_param_list().get_size() - 8;
    actual_param_list.reverse() ; // reverse(). 
	for (std::list<Ast*>::iterator it = actual_param_list.begin(); it != actual_param_list.end(); it++, it1++)
	 {
	 	Code_For_Ast M = (*it)->compile();
	 	list<Icode_Stmt*> L = M.get_icode_list() ; 
	 	for(list<Icode_Stmt *>:: iterator i = L.begin() ; i != L.end() ; i++)
	    ...
	    Register_Addr_Opd* t1 = new Register_Addr_Opd(M.get_reg()) ;
	    string var_name = (*it1) -> get_variable_name() ; 
	    Symbol_Table_Entry* S_ref = new Symbol_Table_Entry( var_name 
	    	, (*it1) -> get_data_type() , (*it1) -> get_lineno() , sp_ref);
	    S_ref -> set_symbol_scope(formal) ; 
	    S_ref -> set_start_offset(-1 * x) ;  
	    Mem_Addr_Opd* Mem = new Mem_Addr_Opd(*S_ref) ;
	    Move_IC_Stmt * Move_stack ;
	    if( (**it1).get_data_type() == int_data_type)
	    {
		    Move_stack = new Move_IC_Stmt(store , t1 , Mem) ;
	    }
	    else
	    {
		    Move_stack = new Move_IC_Stmt(store_d , t1 , Mem) ;
	    }
	    // Move_IC_Stmt * Move_stack = new Move_IC_Stmt(store , t1 , Mem) ;
	    Comb.push_back(Move_stack) ;
	    x += ((*it1) -> get_start_offset() - (*it1) -> get_end_offset()) ; 
	    S_ref -> set_end_offset(-1 * x) ; 
	    M.get_reg() ->  reset_use_for_expr_result() ; // free after storing stack.
	 }
	// sub stack.
	if(x > 0)
	 {
	 	Const_Opd<int> * num = new Const_Opd<int>(x) ;
		Register_Addr_Opd* Stack_pointer = new Register_Addr_Opd(machine_desc_object.spim_register_table[sp]) ;
		Compute_IC_Stmt* Sub_stack = new Compute_IC_Stmt(sub , Stack_pointer , num, Stack_pointer) ; 	 	  	
	 	Comb.push_back(Sub_stack) ; 
	 }

	Label_IC_Stmt* Label1 = new Label_IC_Stmt(jal, procedure_name);
	Comb.push_back(Label1);

	 // add stack.

	actual_param_list.reverse() ; 
	formal_list.reverse() ; // set back.
	 ...
	 if(x > 0)
	 {
	 	...
	 	Const_Opd<int> * num = new Const_Opd<int>(x) ;
		Register_Addr_Opd* Stack_pointer = new Register_Addr_Opd(machine_desc_object.spim_register_table[sp]) ;
		Compute_IC_Stmt* Sub_stack = new Compute_IC_Stmt(add , Stack_pointer , num, Stack_pointer) ; 	 	  	
	 	Comb.push_back(Sub_stack) ; 
	 }
	 ...
	 else if(node_data_type == int_data_type)
	 {
	 	 ...
	 	 return_value_reg = machine_desc_object.spim_register_table[v0] ;
		 Register_Addr_Opd* result = new Register_Addr_Opd(return_value_reg) ;
		 Register_Descriptor* v1_reg = machine_desc_object.spim_register_table[v1];
		 Register_Addr_Opd* operand1 = new Register_Addr_Opd(v1_reg) ;		
		 Move_IC_Stmt* N = new Move_IC_Stmt(mov, operand1,result) ;
		 Comb.push_back(N) ;
	     v1_reg -> reset_use_for_expr_result() ; 
	 }
	 else
	 {
	 	 return_value_reg = machine_desc_object.spim_register_table[f2] ;
		 Register_Addr_Opd* result = new Register_Addr_Opd(return_value_reg) ;
		 Register_Descriptor* f0_reg = machine_desc_object.spim_register_table[f0];
		 Register_Addr_Opd* operand1 = new Register_Addr_Opd(f0_reg) ; 		
		 Move_IC_Stmt* N = new Move_IC_Stmt(move_d, operand1,result) ;
		 Comb.push_back(N) ;
		 f0_reg -> reset_use_for_expr_result() ; 
	 }

	 Code_For_Ast* C = new Code_For_Ast(Comb, return_value_reg);
	 return *C ;
}
