// identifying one error is sufficient.
%{
#include <stdio.h>
#include <stdlib.h>
  extern int yylex(void);	
  void yyerror (char *s);
  extern "C" void yyerror(const char * s) ; 
  extern int yylineno ; 
  Data_Type line_data_type ; 
  Table_Scope scope = global ;  
  Symbol_Table* current_table ;
  Symbol_Table* global_table ;
  int counter = 1;
  Procedure* curr_proc ; 
%}


%union { int integer_value;
		 double double_value;
		 std::string * string_value;
		 list<Ast *> * ast_list;
		 Ast * ast;
		 Symbol_Table * symbol_table;
		 Symbol_Table_Entry * symbol_entry;
		 Basic_Block * basic_block;
		 Procedure * procedure;        
		 Data_Type data_type ;
		 Name_Ast * name_ast ; 
		 Number_Ast <int> * int_ast ;
		 Number_Ast <float> * float_ast ;
		 Arithmetic_Expr_Ast * arith_ast ; 
		 list<Ast*> * l_ast ; 
		 Relational_Op relop ;
		 Logical_Op lop ;  

	 }
%start program
%token <string_value> NAME 
%token <integer_value> INTEGER_NUMBER
%token <string_value> INTEGER
%token <string_value> FLOAT
%token <string_value> VOID
%token <string_value> MAIN 
%token <string_value> ASSIGN
%token <double_value> DOUBLE_NUMBER


// assignment 4 
%token <string_value> WHILE 
%token <string_value> IF
%token <string_value> ELSE 
%token <string_value> DO
%token <string_value> PRINT
%token <string_value> RETURN
%token <relop > RELOP1
%token <relop > RELOP2 
%token <lop> AND 
%token <lop> OR 
%token <lop> NOT 


%nterm <symbol_table>  declarations
%nterm <symbol_table> variable_list 
%nterm <symbol_table> declaration 
%nterm <symbol_table> declaration_list
%nterm <symbol_table> variable_declaration
%nterm <symbol_table> function_arg1
%nterm <symbol_table> function_arg2

// %nterm <symbol_table> declaration_arguments

%nterm <symbol_table> function_def
%nterm <string_value> func_name
%nterm <symbol_table> function_declaration
// doesn't matter, all additions are done to global variables.
%nterm <symbol_table> setup
%nterm <symbol_table> local_declarations
%nterm <data_type> type
%nterm <ast> variable
%nterm <ast> constant
%nterm <ast> term
%nterm <ast> expression 
%nterm <l_ast> statement_list
%nterm <l_ast> expression_list
%nterm <ast> statement
%nterm <ast> assignment_statement
%nterm <ast> while_statement
%nterm <ast> iterative_statement
%nterm <ast> return_statement
%nterm <ast> if_statement
%nterm <ast> print_statement
%nterm <ast> condition_expression
%nterm <ast> question_expression
%nterm <ast> function_call



%right ASSIGN
%left '?' ':'
%left OR 
%left AND 
%left RELOP1
%left RELOP2
%left '+' '-'
%left '*' '/'
%left UMINUS
%right NOT
%nonassoc IFX
%nonassoc ELSE

%nonassoc A1 
%nonassoc A2 
%nonassoc A3 
%nonassoc A4 


%%
program                 :   setup declarations 
							{ 
								program_object.set_global_table(*global_table);
								// printf("global table set.\n");
								//program_object.set_procedure( $2 , yylineno) ;
								// main check.

								if(!program_object.is_procedure_exists("main")  
									|| 	program_object.get_procedure_prototype("main")
									 -> get_return_type() != void_data_type)
								{
									yyerror("cs316: Error : Main function should be declared and it should have int type !!");
									exit(-1) ;
								}
								
							}
		; 

setup 					: /* empty */
						{
							$$ = global_table = new Symbol_Table() ; 
							global_table -> set_table_scope(global) ; 
						}
						; 

declarations            : /* empty */  
							{
							} 
							 | 
							 declaration_list  
							{ 
								// $$ = $1 ;
								// if(scope == global)
								// {   global_table = $$ ;  scope = local ;}
								// else current_table = $$ ; 
							}


		;  

declaration_list       :    declaration 
							 {  
								//$$ = $1 ;
							 }
						|
							declaration_list declaration  
						   { 
								// $1 -> append_list(*$2,yylineno); 
								// $$ = $1 ;
							}
					; 

declaration    :         variable_declaration 
						{
								// global stuff
							 global_table -> append_list( *$1 , yylineno) ;
						}  
						| function_declaration 
						{
							// printf("successfully declared.\n");
						}
						| function_def
		;

variable_declaration             : 
									type variable_list ';' // define scope here.
							 		{
							 			if($1 == int_data_type || $1 == double_data_type)
							 			{
							 				$$ = $2 ;	
							 			}
							 			else
							 			{
							 				yyerror("cs316: Error: variable type can be int or float only\n") ;
							 				exit(-1);
							 			}
							 		}
		;

///////////////////////////////////////////////////////////////

function_declaration    :   type func_name '(' function_arg1 ')' ';'   //can be int or int a
							{
								// if(program_object.is_procedure_exists(*$2))
								// {
								// 	yyerror("cs316: Error: Function already declared!!") ; //exit(-1) ;
								// 	cout << *$2 << "func name\n" ; 	
								// }
								// else
								// {
								// 	//Procedure* P = new Procedure($1, *$2, yylineno);
								// 	//set arguments.
								curr_proc -> set_formal_param_list(*$4); 
								// 	// set procedure to program object.
								// 	//program_object.set_proc_to_map(*$2, P) ;
								// }
							}
						|	type func_name '(' function_arg2 ')' ';'   //can be int or int a
							{
								// if(program_object.is_procedure_exists(*$2))
								// {
								// 	yyerror("cs316: Error: Function already declared!!") ; //exit(-1) ;
								// 	cout << *$2 << "func name\n" ;	
								// }
								// else
								// {
								// 	//Procedure* P = new Procedure($1, *$2, yylineno);
								// 	//set arguments.
								curr_proc -> set_formal_param_list(*$4); 
								// 	// set procedure to program object.
								// 	//program_object.set_proc_to_map(*$2, P) ;
								// }
							}

		;    

// declaration_arguments   :  function_arg1  
// 							{
// 								$$ = $1 ; 
// 							}
// 						|   function_arg2
// 							{
// 								$$ = $1 ; 
// 							}   
// 		;                                

function_arg1           :  /* empty */
							{
								$$ = new Symbol_Table() ;
								$$ -> set_table_scope(formal) ;  
								curr_proc -> set_formal_param_list(*$$);
							}
						|
						 type NAME 
							{
								$$ = new Symbol_Table() ; 
								Symbol_Table_Entry* S = new Symbol_Table_Entry( *$2 , line_data_type , yylineno ) ; 	
								$$ -> push_symbol(S) ; 
								curr_proc -> set_formal_param_list(*$$); 
							}
						|
							function_arg1 ',' type NAME 
							{
								Symbol_Table_Entry* S = new Symbol_Table_Entry( *$4 , line_data_type , yylineno ) ; // fp re	
								$$ = $1 ; 
								$$ -> push_symbol(S) ;
								curr_proc -> set_formal_param_list(*$$); 
							}   
		;
		
function_arg2          :	type
							{
								$$ = new Symbol_Table() ;  
								string temp = "dummy" + to_string(counter);
								counter ++;
								Symbol_Table_Entry* S = new Symbol_Table_Entry( temp , line_data_type , yylineno ) ;
								$$ -> push_symbol(S) ;
								curr_proc -> set_formal_param_list(*$$);
							}                                
						|
								function_arg2 ',' type
							{
								$$ = $1 ; 
								string temp = "dummy" + to_string(counter);
								counter++ ;
								Symbol_Table_Entry* S = new Symbol_Table_Entry( temp , line_data_type , yylineno ) ;
								$$ -> push_symbol(S) ;
								curr_proc -> set_formal_param_list(*$$);
							}    
		;

//////////////////////////////////////////////////////////////////////////////        

type                    :       INTEGER
							{
								line_data_type = int_data_type ;
								$$ = int_data_type ;
							}
						|       FLOAT
							{
								line_data_type = double_data_type ; 
								$$ = double_data_type ; 
							}
						|       VOID
							{
								$$ = void_data_type  ; 
								line_data_type = void_data_type ;
							}
		
		; 

variable_list           :       NAME 
								{
									Symbol_Table_Entry* S = new Symbol_Table_Entry( *$1 , line_data_type , yylineno ) ; // fp references.
									S -> set_symbol_scope(scope) ;
									$$ = new Symbol_Table() ;
									$$ -> set_table_scope(scope) ; 
									$$ -> push_symbol(S) ;
								}
						|      variable_list ','  NAME 
								{
									 Symbol_Table_Entry* S = new Symbol_Table_Entry( *$3 , line_data_type , yylineno ) ; 
									 S -> set_symbol_scope(scope) ;
									 Symbol_Table* SS = new Symbol_Table() ; 
									 SS -> push_symbol(S) ;
									 $1 -> append_list( *SS , yylineno) ; 
									 $$ = $1 ; 
								}
		
		;


function_def          :  type func_name '(' function_arg1 ')' '{'  local_declarations statement_list '}'
						 {
						 		machine_desc_object.clear_local_register_mappings();
						 		// printf("Adding proc to the program\n");
								//Procedure* P = new Procedure( $1, *$2, yylineno) ;
								// take away param list. 
								// take away local list. 
								curr_proc -> set_ast_list(*$8); 	
								curr_proc -> set_proc_is_defined() ; 
								//program_object.set_proc_to_map(*$2, P) ; 
								// printf("set proc\n"); 
								// free the GP registers.

						  }
						
		; 

func_name				: 	NAME
							{
								if(program_object.is_procedure_exists(*$1))
									curr_proc = program_object.get_procedure_prototype(*$1) ; 
								else 
									{
										curr_proc = new Procedure(line_data_type , *$1, yylineno) ;
										program_object.set_proc_to_map(*$1, curr_proc) ;
									}
								if( curr_proc -> is_proc_defined())
								{
									yyerror("cs316 : Error Function defined alreay\n");
								}
								// cout << line_data_type << " " << *$1 << endl ; 
								scope = local ;   
								$$ = $1; 
								
								// set the function procedure.
							}
		;  		

//// local declarations
local_declarations       :    /*empty */ 
							 {  
								$$ = new Symbol_Table() ;
								$$ -> set_table_scope(local) ; 
								current_table = $$ ; 
								curr_proc -> set_local_list(*$$) ;
							 }
						|
							 local_declarations  variable_declaration 
						   { 
								$1 -> append_list(*$2,yylineno);
								current_table = $$ ;  
								$$ = $1 ;
								curr_proc -> set_local_list(*$$) ;
							}
					;

statement_list          :   /* empty */ 
						{ 
							 $$ = new list<Ast*> ;
						}
						|       statement_list 
							   iterative_statement
								{

									$$ = $1 ; 
									$$ -> push_back($2) ; 
								}  
		;   


statement 				: 	 
							 assignment_statement 
							{
								$$ = $1 ; 
							}
							| while_statement
							{
								$$ = $1 ; 
							}
							| if_statement
							{
								$$ = $1 ; 
							}
							| print_statement
							{
								$$ = $1 ;
							}
							| return_statement 
							{
								$$ = $1 ;
							}
							| function_call ';'
							{
								$$ = $1 ;
							}  
							;	

print_statement         :  PRINT variable ';'
							{
								$$ = new Print_Ast($2 , yylineno) ; // assignment 1 ; 
							} ;                             

assignment_statement    :   variable ASSIGN expression ';'
							{
								if( $1 -> get_data_type() == $3 -> get_data_type()) 
								{ 
								   $$  = new Assignment_Ast($1 , $3 , yylineno);                          
								}
								else 
								{ 
								  // cout << "hello" ;
								  yyerror("cs316: Error: I got Heterogeneous data types") ;
								  //exit(-1) ;				
								}
							}        
		;

variable                :   NAME
						  { 
						  	Procedure* prototype = curr_proc ; 
							if( prototype -> variable_in_symbol_list_check(*$1)) 
							{  
								$$ = new Name_Ast(*$1, prototype -> get_symbol_table_entry(*$1), yylineno);
								$$ -> set_data_type((prototype -> get_symbol_table_entry(*$1)).get_data_type())  ;  
							}
							else if(prototype -> variable_in_formal_list_check(*$1))
							{
								$$ = new Name_Ast(*$1, prototype -> get_formal_param_entry(*$1), yylineno);
								$$ -> set_data_type((prototype -> get_formal_param_entry(*$1)).get_data_type())  ; 	
							}
							else
							{
							   if(global_table->variable_in_symbol_list_check(*$1))
							   {
								    $$ = new Name_Ast(*$1, global_table-> get_symbol_table_entry(*$1), yylineno);
									$$ -> set_data_type((global_table -> get_symbol_table_entry(*$1)).get_data_type())  ;
							   }
							   else { 
										yyerror("cs316: Error: No variable declared!!") ;// exit(-1) ;
										// cout << *$1 << " bka\n" ; 
									}
							}
					   }
		
		;

constant                :   INTEGER_NUMBER 
						 { 
								$$ = new Number_Ast <int>($1, int_data_type, yylineno);
							}
						|   DOUBLE_NUMBER
						 {
								$$ = new Number_Ast <double>($1, double_data_type, yylineno);
						 }
			
						;

expression              : expression '+' expression 
						 { 
							 if( $3 -> get_data_type() == $1 -> get_data_type() )
							  {
								  $$ = new Plus_Ast( $1 , $3 , yylineno ) ; 
								  $$ -> set_data_type( $1 -> get_data_type()) ; 
							  }
							  else {
							  yyerror("cs316: Error: Heterogeneous data types.") ; exit(-1) ;
							  }
						} 
						| expression '-' expression 
						{
							   if( $3 -> get_data_type() == $1 -> get_data_type() )
							 {
								 $$ = new Minus_Ast( $1 , $3 , yylineno ) ; 
								 $$ -> set_data_type( $1 -> get_data_type()) ;                           
							 }
							  else { yyerror("cs316: Error: Heterogeneous data types.") ; exit(-1) ; }
				  
						}
						| expression '*' expression 
						{
							 if( $3 -> get_data_type() == $1 -> get_data_type() )
							 {
								$$ = new Mult_Ast( $1 , $3 , yylineno ) ; 
								$$ -> set_data_type( $1 -> get_data_type()) ;                         
							 }
							  else
							  {
								yyerror("cs316: Error: Heterogeneous data types.") ; exit(-1) ;
							}

						}
						| expression '/' expression  
											{
							  if( $3 -> get_data_type() == $1 -> get_data_type() )
							 {
								$$ = new Divide_Ast( $1 , $3 , yylineno ) ; 
								$$ -> set_data_type( $1 -> get_data_type()) ;                           
							  }
							  else
							 {
								yyerror("cs316: Error: Heterogeneous data types.") ; exit(-1) ; 
							}

						}
						| question_expression 
						{
							$$ = $1 ;
						} 
						| '-' expression %prec UMINUS
						{
							 $$ = new UMinus_Ast($2,NULL,yylineno);
							 $$ -> set_data_type( $2 -> get_data_type()) ; 
						}
						| term 
						{   $$ = $1 ; }
						;



term                  : variable 
						 {
							 $$ = $1 ;
						 }
						| constant 
						 {
							  $$ = $1 ;
						 }
						 | '(' expression ')'
						 {
							$$ = $2 ; 
						 }
						 |
						 	function_call
					 	 {
					 	 	$$ = $1 ;
			 	    	 }
				;

function_call			:	NAME '(' expression_list ')'
							{
								// do a sanity check before.
								// is there in the map ??
								if(!program_object.is_procedure_exists(*$1))
								{
									yyerror("cs316: Error: unknown function called") ; 
									exit(-1) ;
								}
								// check argument sanity
								Procedure* prototype = program_object.get_procedure_prototype(*$1) ; 
							 	Call_Ast* func_ast = new Call_Ast( *$1, yylineno);
							 	// set the function parameters
							 	func_ast -> set_actual_param_list(*$3) ; 
							 	// check arguments...
							 	func_ast -> check_actual_formal_param( prototype -> get_formal_param_list()) ; 
							 	// set return type
							 	func_ast -> set_data_type( prototype -> get_return_type()) ; 
							 	// printf("function data type %d", func_ast -> get_data_type()) ; 
							 	// set return reg ??
							 	//exit(-1) ; 
							 	$$ = func_ast ; 
							}
						|	
							NAME '(' ')'
							{
								if(!program_object.is_procedure_exists(*$1))
								{
									yyerror("cs316: Error: unknown function called") ; 
								}
								Procedure* prototype = program_object.get_procedure_prototype(*$1) ; 								
								Call_Ast* func_ast = new Call_Ast( *$1, yylineno);
								Symbol_Table * st = new Symbol_Table() ;
								// func_ast -> set_actual_param_list(*st) ;
							 	func_ast -> check_actual_formal_param( *st) ; 
								func_ast -> set_data_type(prototype -> get_return_type()) ;							 	
							 	//printf("function data type %d", func_ast -> get_data_type()) ; 

							 	// no function parameters
							 	// func_ast -> set_actual_param_list(*$3) ; 
							 	$$ = func_ast ; 	
							}

				;

expression_list			:  expression // arguments to function.
						{
							$$ = new list<Ast*>() ;
							$$ -> push_back($1) ;  
						}
						| expression_list ',' expression
						{
							$$ = $1 ; 
							$$ -> push_back($3) ; 		
						}
						;					

condition_expression    :  expression RELOP1 expression 
							{
								// lhs, relop, rhs, line
								 $$ = new Relational_Expr_Ast( $1 , $2 , $3 , yylineno);  
								 if($$ -> check_ast() == 0)
								 {
									yyerror("cs316: Error: Relational expression data types didn't match") ; 
									exit(-1) ; 
								 }
								  $$ -> set_data_type(int_data_type) ; 
								 // printf("Done1\n");    		
							}
						|  expression RELOP2 expression
							{
								 $$ = new Relational_Expr_Ast( $1 , $2 , $3, yylineno) ; 
								 if($$ -> check_ast() == 0)
								 {
									yyerror("cs316: Error: Relational expression data types didn't match") ; 
									exit(-1) ; 
								 }
								$$ -> set_data_type(int_data_type) ; 
								  // printf("Done2\n");            
							}
						|  condition_expression AND condition_expression
							{
								// lhs,  bool_op, rhs, line no.
								$$ = new Logical_Expr_Ast($1 , $2 , $3 , yylineno) ; 

								 if($$ -> check_ast() == 0)
								 {
									yyerror("cs316: Error: Logical_Op expression data types didn't match") ; 
									exit(-1) ; 
								 }
								 $$ -> set_data_type(int_data_type) ; 
							}
						|  condition_expression OR condition_expression
						{
							  $$ = new Logical_Expr_Ast($1 , $2 , $3 , yylineno) ;
								 if($$ -> check_ast() == 0)
								 {
									yyerror("cs316: Error: Logical_Op expression data types didn't match") ; 
									exit(-1) ; 
								 }
							 $$ -> set_data_type(int_data_type) ;  
						}
						|  NOT condition_expression
						{
							$$ = new Logical_Expr_Ast(NULL , $1 , $2 , yylineno) ; 
							 if($2 == NULL)
							 {
								yyerror("cs316: Error: Logical_Op expression error") ; 
								exit(-1) ; 
							 }
						   $$ -> set_data_type(int_data_type) ; 
						}
						| '(' condition_expression ')'
						{
							$$ = $2 ; 
						}

						; 

question_expression      : condition_expression '?' expression ':' expression 
						  {
							$$ = new Conditional_Expression_Ast($1,$3,$5,yylineno) ;
							if($$ -> check_ast() == 0) 
							{
								yyerror("cs316: Error: Heterogeneous data types.") ;
								exit(-1) ; 
							} 
							// set data type of condition expressions
							$$ -> set_data_type($3->get_data_type()) ;
							//printf("%d\n", $$ -> get_data_type() ); 
						  }
						; 

if_statement            : IF '(' condition_expression ')'  iterative_statement   ELSE
							iterative_statement 
							 {
								// cond, then, else, line no
								$$ = new Selection_Statement_Ast($3 , $5 , $7 ,yylineno) ; 
							 }
						  | IF '(' condition_expression ')'  iterative_statement    %prec IFX  
							{
								$$ = new Selection_Statement_Ast($3 , $5 , NULL ,yylineno) ; 
							}
							;


iterative_statement     : statement 
						{
							$$ = $1  ; 
						}
						|   '{'  statement_list '}'
						{
							Sequence_Ast * S = new Sequence_Ast(yylineno) ;
							list<Ast*> X = *$2 ; 
							if(X.size() == 0) 
							{
								yyerror("cs316: Error: Empty Block error") ; 
								exit(-1) ; 
							}
							for(list<Ast*>::iterator i = X.begin() ; i != X.end() ; i++)
								{
									S -> ast_push_back(*i); 
								}

							$$ = S ;  
						}
						; 


while_statement			: WHILE '(' condition_expression ')' iterative_statement 
							{
								// what about the types ...
								// cond, body, line, bool
								$$ = new Iteration_Statement_Ast($3 , $5 , yylineno, false) ; 
							}
						  |
						  DO iterative_statement WHILE '(' condition_expression ')' ';'
						  {
								// Sequence_Ast * S = new Sequence_Ast(yylineno) ;
								// list<Ast*> X = *$2 ; 
								// for(list<Ast*>::iterator i = X.begin() ; i != X.end() ; i++)
								// {
								//     S -> ast_push_back(*i); 
								// } 
							 $$ = new Iteration_Statement_Ast($5 ,$2 , yylineno, true) ; 
						  }
							; 
return_statement		: RETURN expression ';'
							{
								// create return AST.
								if(curr_proc -> get_return_type() == void_data_type)
								{
									yyerror("cs316: Error The return has to be void!!\n") ;
									exit(-1);
								}

								if(curr_proc -> get_return_type() != $2 -> get_data_type())
								{
									yyerror("cs316: Error The return type mismatch!!\n") ;
									exit(-1) ;
								}

								$$ = new Return_Ast(  $2 , curr_proc -> get_proc_name() ,  yylineno) ;
								$$ -> set_data_type( curr_proc -> get_return_type()) ;

							} 
						 ; 


%%

void yyerror (char*  s)
{
  printf("%s" , s) ; 
}

extern YYSTYPE yylval ; 
